...
const char* const kSplashAnimationFinished = "splashAnimationFinished";
const char* const kSplashAnimationFinishedSignature = "()V";
...
jmethodID sSplashAnimationFinished = nullptr;
}
...
VRBrowser::InitializeJava(JNIEnv* aEnv, jobject aActivity) {
  ...
  sSplashAnimationFinished = FindJNIMethodID(sEnv, sBrowserClass, kSplashAnimationFinished, kSplashAnimationFinishedSignature);
}
...
void
VRBrowser::SplashAnimationFinished() {
  if (!ValidateMethodID(sEnv, sActivity, sSplashAnimationFinished, __FUNCTION__ )) { return; }
  sEnv->CallVoidMethod(sActivity, sSplashAnimationFinished);
  CheckJNIException(sEnv, __FUNCTION__ );
}
