...
public class SettingsStore {
    ...
    //Kaidi,默认设置为桌面模式；需要注意：修改点不仅仅是这里，还有SessionSettings中的withDefaultSettings()方法
    //补充：还要修改桌面模式按钮的处理逻辑，在NavigationBarWidget.onSwitchMode以及Session.trySetUaMode
    public final static int UA_MODE_DEFAULT = WSessionSettings.USER_AGENT_MODE_DESKTOP;
    public final static int INPUT_MODE_DEFAULT = 1;
    public final static float DISPLAY_DENSITY_DEFAULT = 1.0f;
    //Kaidi,修改屏幕大小，这个大小不改变在空间中的位置以及大小；变大只会让屏幕的内容更加细腻
    public final static int WINDOW_WIDTH_DEFAULT = 1200;
    public final static int WINDOW_HEIGHT_DEFAULT = 675;
    ...
    // 注意这里颜色的顺序，RGBA
    public final static int POINTER_COLOR_DEFAULT_DEFAULT = Color.parseColor("#8BF6FFFF");
    public final static int SCROLL_DIRECTION_DEFAULT = 0;
    //Kaidi,修改skybox类型。 Wolvic默认使用cyberpunk
    ...
    public final static boolean POP_UPS_BLOCKING_DEFAULT = false;
