...
public class Session implements WContentBlocking.Delegate, WSession.NavigationDelegate,
    ...
    private boolean trySetUaMode(int mode) {
        ...
        if (mode == WSessionSettings.USER_AGENT_MODE_DESKTOP) {
            //Kaidi，目前VIEWPORT_MODE_DESKTOP，会显示横向滚动条，因此还是使用VIEWPORT_MODE_MOBILE
    ...
    WResult<WAllowOrDeny> onLoadRequest(@NonNull WSession aSession, @NonNull LoadRequest aRequest) {
        ...
        //Kaidi,这段逻辑是拦截打开新标签页。一旦检测到打开新标签页，通过本Session直接加载
        //并且返回WResult.deny。可能会与历史列表冲突，也许以后会移除
        if (aRequest.target == WSession.NavigationDelegate.TARGET_WINDOW_NEW
                && !uri.isEmpty()
                && !uri.equalsIgnoreCase(mContext.getString(R.string.about_blank))) {
            this.loadUri(aRequest.uri);
            return WResult.deny();
        }
