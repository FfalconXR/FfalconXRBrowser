...
import static com.igalia.wolvic.LauncherIntentKt.DOF_0;
import static com.igalia.wolvic.LauncherIntentKt.ICONS_TEMPLATE_DEFAULT;
import static com.igalia.wolvic.LauncherIntentKt.INTENT_URI_DATA;
...
public class VRBrowserActivity extends PlatformActivity implements WidgetManagerDelegate,
    ...
    LegalDocumentDialogWidget mTermsServiceDialog = null;
    LegalDocumentDialogWidget mPrivacyPolicyDialog = null;
    ...
    private String mOriginalUrlFromLauncher = "";
    ...
    protected void initializeWidgets() {
        ...
        mWindows.setDelegate(new Windows.Delegate() {
            ...
            public void onFocusedWindowChanged(@NonNull WindowWidget aFocusedWindow, @Nullable WindowWidget aPrevFocusedWindow) {
                ...
                if (aPrevFocusedWindow != null) {
                    aPrevFocusedWindow.onPause();
                }
            }
        ...
        // Create Browser navigation widget
        mNavigationBar.addListeners(mWindows);
        // Create the tray
...
//        }
    ...
    private void attachToWindow(@NonNull WindowWidget aWindow, @Nullable WindowWidget aPrevWindow) {
        ...
        if (mTermsServiceDialog != null) {
            mTermsServiceDialog.attachToWindow(aWindow);
        }
        if (mPrivacyPolicyDialog != null) {
            mPrivacyPolicyDialog.attachToWindow(aWindow);
        }
        ...
        if (aPrevWindow != null) {
            ...
            updateWidget(mTray);
            if (mTermsServiceDialog != null) {
                updateWidget(mTermsServiceDialog);
            }
            if (mPrivacyPolicyDialog != null) {
                updateWidget(mPrivacyPolicyDialog);
            }
        }
    }
    ...
    private boolean showTermsServiceDialogIfNeeded() {
        ...
        mTermsServiceDialog.setDelegate(response -> {
            if (response) {
...
//                    showWhatsNewDialogIfNeeded();
        ...
        mTermsServiceDialog.attachToWindow(mWindows.getFocusedWindow());
        mTermsServiceDialog.show(UIWidget.REQUEST_FOCUS);
        return true;
    }
    ...
    private boolean showPrivacyDialogIfNeeded() {
        ...
        mPrivacyPolicyDialog.setDelegate(response -> {
            if (response) {
...
//                showWhatsNewDialogIfNeeded();
        ...
        mPrivacyPolicyDialog.attachToWindow(mWindows.getFocusedWindow());
        mPrivacyPolicyDialog.show(UIWidget.REQUEST_FOCUS);
        return true;
    }
    ...
    void loadFromIntent(final Intent intent) {
        ...
        if (uri == null) {
            Log.w(LOGTAG, "Load default if uri is null");
            loadHome();
            mOriginalUrlFromLauncher = "";
            return;
        }

        if (uri.getScheme() == null || !uri.getScheme().equalsIgnoreCase(getString(R.string.intent_scheme))) {
            Log.d(LOGTAG, String.format("URI scheme=%s is null or is not %s", uri.getScheme(), getString(R.string.intent_scheme)));
            ...
            Log.d(LOGTAG, "===== Load from intent --> load url=" + uri);
            enter3DofScreen();
            showIconsTemplate(ICONS_TEMPLATE_DEFAULT);
        ...
        Log.d(LOGTAG, "Loading URI from intent: " + uri);

        String data = uri.getQueryParameter(INTENT_URI_DATA);

        if (data == null || data.isEmpty()) {
            Log.w(LOGTAG, "data from intent is null or empty");
            loadHome();
            mOriginalUrlFromLauncher = "";
            return;
        }

        // Start resolving intent data..

        byte[] decoderArray = Base64.decode(data, Base64.DEFAULT);
        String base64Decoder = new String(decoderArray, Charsets.UTF_8);
        Log.d(LOGTAG, "base64 decoder=" + base64Decoder);
        if (mGson == null) {
            mGson = new Gson();
        }
        LauncherIntent launcherIntent = mGson.fromJson(base64Decoder, LauncherIntent.class);

        if (launcherIntent == null || launcherIntent.getUrl().isEmpty()) {
            Log.w(LOGTAG, "JSON can not be resolved or the URL is empty");
            loadHome();
            mOriginalUrlFromLauncher = "";
            return;
        }

        WindowWidget window = getFocusedLauncherFullscreenWindow();
        Session oldSession = window.getSession();
        // 为了返回按钮最多只能返回到由 Launcher 传入的那个 URL，所以在这里新建一个 session
        Session newSession = SessionStore.get().createSession(false);
        window.setSession(newSession, WindowWidget.DEACTIVATE_CURRENT_SESSION);
        newSession.setUaMode(launcherIntent.getUserAgentMode());
        newSession.loadUri(launcherIntent.getUrl());

        if (launcherIntent.getDof() == DOF_0) {
            Log.d(LOGTAG, "===== Load from intent --> 0DOF fullscreen");
            enter0DofFullscreen();
        } else {
            Log.d(LOGTAG, "===== Load from intent --> 3DOF");
            enter3DofScreen();
        }
        mOriginalUrlFromLauncher = launcherIntent.getUrl();
        DataReportHelper.reportUrl(this, mOriginalUrlFromLauncher);
        showIconsTemplate(launcherIntent.getIconsTemplate());
        // 在创建 WindowWidget 时会同时新建 uri 为 about:blank 的 session，
        // 在这里将其销毁，从而避免在 3DOF 窗口中看到很多无意义的 tab
        if (oldSession != null) {
            SessionStore.get().destroySession(oldSession);
        }
    }

    private void loadHome() {
        Log.d(LOGTAG, "===== Load from intent --> load home");
        showIconsTemplate(ICONS_TEMPLATE_DEFAULT);
        enter3DofScreen();
        mWindows.getFocusedWindow().loadHomeIfBlank();
    }

    private void enter0DofFullscreen() {
        getFocusedWindow().enterFullscreenSize();
    ...
    private void enter3DofScreen() {
        closeLauncherFullscreenWindow();
        getTray().setVisible(true);
        getNavigationBar().setVisible(true);
        enter3DofMode();
        updateEnvironment();
    }

    private WindowWidget getFocusedLauncherFullscreenWindow() {
        for (WindowWidget windowWidget : mWindows.getCurrentWindows()) {
            if (windowWidget.isLauncherFullscreenMode()) {
                Log.d(LOGTAG, "reuse launcher fullscreen window");
                mWindows.focusWindow(windowWidget);
                return windowWidget;
            }
        }

        if (canOpenNewWindow()) {
            Log.d(LOGTAG, "Can open new window");
            // Will focus the new window
            return mWindows.addWindow();
        }

        Log.w(LOGTAG, "Can not open new window, current windows count: " + mWindows.getCurrentWindows().size());
        ...
        for (WindowWidget windowWidget : mWindows.getCurrentWindows()) {
            ...
            if (window == null || window.getSession().getLastUse() > session.getLastUse()) {
                window = windowWidget;
            }
        }
        Windows.WindowPlacement windowPlacement = window.getWindowPlacement();
        if (windowPlacement == Windows.WindowPlacement.LEFT) {
            mWindows.moveWindowRight(window);
        } else if (windowPlacement == Windows.WindowPlacement.RIGHT) {
            mWindows.moveWindowLeft(window);
        }
        getWindows().focusWindow(window);
        Log.d(LOGTAG, "Reuse LRU window");
        return window;
    }

    public String getOriginalUrlFromLauncher() {
        return mOriginalUrlFromLauncher;
    }

    private void closeLauncherFullscreenWindow() {
        Log.d(LOGTAG, "closeLauncherFullscreenWindow()");
        if (mWindows == null) {
            return;
        }
        WindowWidget fullscreenWindow = null;
        for (WindowWidget windowWidget : mWindows.getCurrentWindows()) {
            Log.d(LOGTAG, "window title=" + windowWidget.getSession().getCurrentTitle() + ", launcherMode=" + windowWidget.getLauncherMode());
            if (windowWidget.isLauncherFullscreenMode()) {
                fullscreenWindow = windowWidget;
                break;
            }
        }
        if (fullscreenWindow != null) {
            Log.d(LOGTAG, "close fullscreen window");
            mWindows.closeWindow(fullscreenWindow);
        }
    }
                ...
                mWindows.getFocusedWindow().showConfirmPrompt(
                        ...
                        new String[]{
                            ...
                            if (index == PromptDialogWidget.POSITIVE) {
                                ...
                                VRBrowserActivity.super.finish();
                            }
                        });
    ...
    void onEnterWebXR() {
        ...
        if (getFocusedWindow().isLauncherFullscreenMode()) {
            enter3DofMode();
        }
    ...
    public String getActiveEnvironment() {
        String env;
        if (getFocusedWindow().isLauncherFullscreenMode()) {
            env = getServicesProvider().getEnvironmentsManager().getOrDownloadEnvironment("void");
        } else {
            env = getServicesProvider().getEnvironmentsManager().getOrDownloadEnvironment();
        }
        return env;
    }
    ...
    //该函数由PlatformActivity调用，因此不需要再runOnUiThread()
    public void dispatchWidgetScrollEvent(float dx, float dy) {
        //Kaidi，如果有其他的activeDialog，就滚动该Dialog
        if(mActiveDialog != null && mActiveDialog.isVisible()) {
            int handle = mActiveDialog.getHandle();
            handleScrollEvent(handle, 0, dx / 30, dy / 30);
            return;
        }

        //原来的滚动，是通过MotionEvent来传递，这里除以30是为了避免滚动过快
        if (mWindows.getFocusedWindow() != null) {
            int handle = mWindows.getFocusedWindow().getHandle();
            handleScrollEvent(handle, 0, dx / 30, dy / 30);
        }
    }

    public void hideKeyboardWidget() {
        if (mKeyboard != null)
    ...
    private void splashAnimationFinished() {
        runOnUiThread(this::splashFinished);
    }
    ...
    public void setControllersVisible(final boolean aVisible) {
        Log.d(LOGTAG, "setControllersVisible() --> aVisible=" + aVisible);
