...
public class NavigationBarWidget extends UIWidget implements WSession.NavigationDelegate,
    ...
    private void updateUI() {
        ...
        mBinding.navigationBarNavigation.tabsButton.setOnClickListener(view -> {
        ...
        mBinding.navigationBarNavigation.bookmarkButton.setOnClickListener(view -> {
            view.requestFocusFromTouch();
            handleBookmarkClick();
        });
    ...
    private Observer<ObservableBoolean> mIsBookmarkedObserver = aBoolean -> mBinding.navigationBarNavigation.bookmarkButton.clearFocus();
    ...
    private void handleBookmarkClick() {
        ...
        String url = getSession().getCurrentUri();
        ...
        bookmarkStore.isBookmarked(url).thenAcceptAsync(bookmarked -> {
            if (!bookmarked) {
                bookmarkStore.addBookmark(url, getSession().getCurrentTitle());
    ...
    public void onShowAwesomeBar() {
        ...
        mSuggestionsProvider.getSuggestions()
                .whenCompleteAsync((items, ex) -> {
                    //Kaidi，原先的isAutocompleteEnabled()逻辑，仅仅是禁用了SearchEngineWrapper，此外还有历史以及BookMark的Suggestion
                    //因此原先即使禁用了地址栏自动补全，还是会有提示
                    //需要在这里添加处理逻辑：只有当isAutocompleteEnabled()，才会弹出来
                    if (mBinding.navigationBarNavigation.urlBar.hasFocus() && SettingsStore.getInstance(getContext()).isAutocompleteEnabled()) {
...
//                    final int defaultUaMode = SettingsStore.getInstance(mAppContext).getUaMode();
                    //Kaidi,ԭ��Ĭ�ϲ���Desktop��������л��߼���Ҫ�޸�
                    mHamburgerMenu.setUAMode(WSessionSettings.USER_AGENT_MODE_VR);
                    mAttachedWindow.getSession().setUaMode(WSessionSettings.USER_AGENT_MODE_VR);
