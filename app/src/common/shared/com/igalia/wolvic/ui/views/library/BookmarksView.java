...
public class BookmarksView extends LibraryView implements BookmarksStore.BookmarkListener {
    ...
    public void updateUI() {
        ...
        mBinding.title.setText(getContext().getString(R.string.bookmarks_title));
    ...
    private void showBookmarks(List<BookmarkNode> aBookmarks) {
        if (aBookmarks == null || aBookmarks.size() == 0) {
            ...
            mViewModel.setIsLoading(false);
            List<BookmarkNode> newBookmarks = new ArrayList<>();
            for (int i = 0; i < aBookmarks.size(); i++) {
                BookmarkNode bookmarkNode = aBookmarks.get(i);
                if(bookmarkNode.getGuid().equals(BookmarkRoot.Mobile.getId())){
                    newBookmarks.add(bookmarkNode);
                }
            }
            mBookmarkAdapter.setBookmarkList(newBookmarks);
        }
