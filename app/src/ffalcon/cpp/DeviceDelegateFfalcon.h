...
class DeviceDelegateFfalcon;
typedef std::shared_ptr<DeviceDelegateFfalcon> DeviceDelegateFfalconPtr;

class DeviceDelegateFfalcon : public DeviceDelegate {
public:
  static DeviceDelegateFfalconPtr Create(vrb::RenderContextPtr& aContext);
  ...
  // DeviceDelegateFfalcon interface
...
protected:
  struct State;
  DeviceDelegateFfalcon(State& aState);
  virtual ~DeviceDelegateFfalcon();
private:
  State& m;
  float controllerPos[4];
  VRB_NO_DEFAULTS(DeviceDelegateFfalcon)
};
