...
static crow::DeviceDelegateFfalconPtr sDevice;
...
(JNIEnv* aEnv, jobject aActivity, jobject aAssetManager) {
  if (!sDevice) {
    sDevice = crow::DeviceDelegateFfalcon::Create(BrowserWorld::Instance().GetRenderContext());
  }
  sDevice->Resume();
...
(JNIEnv*, jobject, jboolean aDown, jfloat aX, jfloat aY) {
  //Kaidi, touch event will NOT need any more!!!!
...
/*****************
 * Kaidi JNI device/controller quaternion
 ****************/
JNI_METHOD(void, setDeviceQuaternion)
(JNIEnv*, jobject, float f0, float f1, float f2, float f3) {
  sDevice->SetDeviceQuaternion(f0, f1, f2, f3);
}

JNI_METHOD(void, setControllerQuaternion)
(JNIEnv*, jobject, float f0, float f1, float f2, float f3) {
  sDevice->SetControllerQuaternion(f0, f1, f2, f3);
}
